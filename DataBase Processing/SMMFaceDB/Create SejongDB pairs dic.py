...
def create_pairs(files_dic):                        # need a dictionary for thermal and visible image pairs, paths and subject id
    ...
    for v_key in files_dic:
        if '_v.jpg' in v_key:                          # check only visible images    
            ther_key = v_key.replace('_v.jpg','_t.jpg')
            viir_key = v_key.replace('_v.jpg','_l.jpg')
            infr_key = v_key.replace('_v.jpg','_d.jpg')
            
            if ther_key in files_dic:
                if viir_key in files_dic:
                    if infr_key in files_dic:
                        vis_img_path = files_dic[v_key][0]
                        the_img_path = files_dic[ther_key][0]
                        viir_img_path = files_dic[viir_key][0]
                        infr_img_path = files_dic[infr_key][0]
                        
                        label = files_dic[v_key][1]
                        img_pairs_dic[img_no] = [label, vis_img_path, the_img_path, viir_img_path, infr_img_path]
                        img_no += 1
...
file_path = 'SejongDB_Images.txt' 
files_dic = read_Json(file_path)
img_pairs_dic = create_pairs(files_dic)
write_Jason(img_pairs_dic,"SejongDB Img Quads.txt")